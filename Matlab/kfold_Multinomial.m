function error = kfold_Multinomial(labels_train, features_train, labels_test, features_test) 
...
for i = 1:K
    ...
    for j = 1:26
        ...
        likelihood(j,:) = (1 + sum(features))./(sum(sum(features)) + 16);
    end
    
    for j = 1:length(labels_validation)
        for k = 1:26
            check(k) = log(prior(k)) + sum(log(likelihood(k,:)).*features_validation(j,:));
        end
